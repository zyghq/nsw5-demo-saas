(function () {
  ...
  function loadWidgetConfig(widgetId) {
    console.log("Todo: API request widgetId", widgetId);
    const response = {
      ...
      // bot_message_bg_color: "#EEEEF1",loader
      // bubble_color: "#EEEEF1",
  ...
  function createZygWidget(config, authToken) {
  ...
  var loadZygWidget = function (widgetId, authToken) {
    window.clearInterval(intervalId);
    loadWidgetConfig(widgetId)
      .then((config) => {
        createZygWidget(config, authToken),
  ...
  intervalId = window.setInterval(function () {
    if (
      ...
      window.zyg &&
      window.zyg.widgetId &&
      window.zyg.authToken
    ) {
      var authToken = window.zyg.authToken;
      var widgetId = window.zyg.widgetId; // fetch widget config
      if (authToken && widgetId) {
        widgetLoaded = true;
        loadZygWidget(widgetId, authToken);
      }
    }
  }, 1000);
})();
